...
public class PermissionManager {
    ...
    private final PermissionIsolationManager isolationManager;
    ...
    public PermissionManager(PermissionChecker permissionChecker) {
        ...
        this.activeRequests = new ConcurrentHashMap<>();
        this.mainHandler = new Handler(Looper.getMainLooper());
        this.isolationManager = new PermissionIsolationManager(
            permissionChecker != null ? permissionChecker.getApplicationContext() : null
        );
    }
    ...
    public String requestPermissions(RequestContext context) {
        ...
        GrantlyLogger.d(TAG, new GrantlyLogger.LogBuilder()
            ...
            .append("IsolationStatus", isolationManager.getSystemStatus())
            .build());
        
        // CRITICAL SAFETY CHECK: Validate request through isolation manager
        if (!isolationManager.validatePermissionRequest(permissions)) {
            GrantlyLogger.e(TAG, "Permission request failed isolation validation - blocking for safety");
            handleFailedValidation(context);
            return requestId;
        }
        
        // Record request start for tracking
        isolationManager.recordRequestStart(requestId, permissions);
        ...
        // Perform system integrity check before proceeding
        if (!isolationManager.performSystemIntegrityCheck()) {
            GrantlyLogger.e(TAG, "System integrity check failed - entering recovery mode");
            isolationManager.triggerSystemRecovery("Pre-request integrity check failed");
            handleSystemRecovery(context);
            return requestId;
        }
        ...
        try {
            ...
            if (!permissionsToRequest.isEmpty()) {
                ...
                // Additional safety check on filtered permissions
                if (!isolationManager.validatePermissionRequest(permissionsArray)) {
                    GrantlyLogger.e(TAG, "Filtered permissions failed validation - aborting request");
                    handleFailedValidation(context);
                    cleanup(requestId);
                    return requestId;
                }
                ...
                // CRITICAL: Final safety check before making system call
                if (!isolationManager.performSystemIntegrityCheck()) {
                    GrantlyLogger.e(TAG, "Final integrity check failed - aborting request");
                    isolationManager.triggerSystemRecovery("Pre-system-call integrity check failed");
                    handleSystemRecovery(context);
                    cleanup(requestId);
                    return requestId;
                }
                
                // Request the permissions - this is the critical system call
                GrantlyLogger.d(TAG, "Making system permission request for " + permissionsArray.length + " permissions");
                ...
                GrantlyLogger.d(TAG, "System permission request completed successfully");
            }
            
            return requestId;
            
        } catch (Exception e) {
            // Handle any unexpected errors during permission request
            GrantlyLogger.e(TAG, "Error during permission request: " + e.getMessage(), e);
            isolationManager.triggerSystemRecovery("Exception during permission request: " + e.getMessage());
            isolationManager.recordRequestComplete(requestId);
            handleSystemRecovery(context);
            cleanup(requestId);
            return requestId;
        }
    ...
    public boolean onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        GrantlyLogger.d(TAG, "Received permission result - RequestCode: " + requestCode + 
        ...
        if (matchingContext == null) {
            GrantlyLogger.w(TAG, "No matching request context found for permission result");
            return false;
        }
        
        try {
            // Record request completion
            isolationManager.recordRequestComplete(matchingContext.getRequestId());
            ...
            GrantlyLogger.d(TAG, "Permission result processed successfully");
            return true;
            
        } catch (Exception e) {
            // Handle any errors during result processing
            GrantlyLogger.e(TAG, "Error processing permission result: " + e.getMessage(), e);
            isolationManager.triggerSystemRecovery("Error processing permission result: " + e.getMessage());
    ...
    /**
     * Handles failed validation by invoking error callback.
     *
     * @param context The request context
     */
    private void handleFailedValidation(RequestContext context) {
        GrantlyCallback callback = context.getCallback();
        if (callback != null) {
            mainHandler.post(() -> {
                List<PermissionResult> errorResults = new ArrayList<>();
     ...
     * @param context The request context
     */
    private void handleSystemRecovery(RequestContext context) {
        GrantlyCallback callback = context.getCallback();
        if (callback != null) {
            mainHandler.post(() -> {
                // Report all permissions as denied during recovery
                List<PermissionResult> recoveryResults = new ArrayList<>();
                for (String permission : context.getPermissions()) {
                    recoveryResults.add(new PermissionResult(permission, PermissionState.DENIED, false));
                }
                callback.onPermissionResult(recoveryResults);
            });
        }
    }
    
    /**
     * Gets the isolation manager for testing purposes.
     *
     * @return The current PermissionIsolationManager instance
     */
    PermissionIsolationManager getIsolationManager() {
